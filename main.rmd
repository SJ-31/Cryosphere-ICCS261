...
Now we will perform tests on the abundance of identified OTUs within the sites
```r
...
ancombc_lfc <- read_csv(glue("{abc_path}/export/w_slice.csv")) %>%
...
# Log-fold change divided by standard error
ancombc_q_val <- read_csv(glue("{abc_path}/export/w_slice.csv")) %>%
    select(.data = ., !`(Intercept)`)
