...
- Differential abundance was performed against the different habitat
...
ANCOM-BC test returns p-values, so the results here are only the log-fold changes that are statistically significant
...
- Compared to Glacier sites, Permafrost has a higher abundance of the CENTBENZCOA pathway and the
...
As an extension to one of the assignments, I wanted to build a model that predicts multiple classes
- Habitat type ("Permafrost", "Glacier" or "Other") was the only variable that made sense for the dataset
  - Predicting Location would have worked as well, but I didn't have enoguh samples from each
