...
# Functional analysis
The raw output tables here are the absolute abundances of predicted biological pathways in each site using the KEGG Orthology database (KO). PICRUSt2 integrates with other databases, such as Enzyme Commission numbers (EC), but I've chosen to use the KO database because the PICRUSt2 authors describe how they've improved on the database integration compared to PICRUSt1.
...
# chosen_paths <-c("chlorophyll-syn", "glycolysis", "tca", "")
nice_paths <- ko_all %>%
  filter(!(grepl("[0-9]", pathway)))
...
stacked_paths <- path_sum %>% ggplot(., aes(x = name, y = value, fill = identifier)) +
  geom_bar(stat = "identity") +
  scale_fill_discrete(name = "Pathways") +
  scale_color_paletteer_d("pals::glasbey") +
  labs(
    x = "Site", y = "Relative abundance",
  )
ggsave("./figures/path_plot.png", stacked_paths, width = 20, height = 10, dpi = 300)
```
