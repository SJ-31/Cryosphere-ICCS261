...
## Exploring the jellyfish microbiome

### Sample background
The data consists of umbrella, oral arms and secreted mucus samples from the jellyfish species *Rhizostoma pulmo*. *R pulmo* belongs to the Rhizhostomatidae family of the class Scyphozoa, and is commonly found in the Mediterranean sea  [@mariottiniMediterraneanJellyfishVenoms2010]. This species is large, but not particularly venomous; it is considered "completely harmless to humans" [@mariottiniMediterraneanJellyfishVenoms2010].
...
genus_plot <- rel_abund %>% ggplot(aes(x = source, y = Genus, fill = value)) +
  geom_tile() +
...
genus_plot
key <- c(Umbrella = "J1", Mucus = "J2", Arms = "J3")
venn <- summed_otus %>%
  ...
  rename(., all_of(key)) %>%
  mutate(Genus = summed_otus$Genus) %>%
  ggvenn(., c("Umbrella", "Mucus", "Arms"), show_elements = "Genus", text_size = 2)
  # ggplot(., aes(A = Umbrella, B = Mucus, C = Arms)) +
  # geom_venn( show_elements = TRUE)
venn
```
From the
...
```{r}
file <- read_delim("./results/3-FunctionAnnotation/Jelly_PICRUST2/pathways_out/path_abun_unstrat.tsv", delim = "\t")
pathways <- file %>%
  as.data.frame() %>%
  t() %>%
  `colnames<-`(subset(., grepl("pathway", rownames(.)))) %>%
  as.data.frame() %>%
  slice(-1) %>%
